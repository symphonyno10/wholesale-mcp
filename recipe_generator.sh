...
■ MCP 도구만 사용하라 (Read/Grep/Glob/Edit/Bash 사용 금지):
  - 크레덴셜 확인: get_session_info() → login_data에 VEN_CD 등 포함
  - 레시피 읽기: get_recipe(site_id) → 저장된 레시피 JSON 반환
  - 레시피 저장: save_recipe(site_id, recipe_json, overwrite=True)
  - 사이트 목록: list_sites() → 등록된 사이트 + 로그인 상태
  - 파일을 직접 읽거나 수정하지 마라. 위 MCP 도구로 전부 가능하다.

■ 레시피 검증 필수: save_recipe() 후 반드시 get_recipe()로 확인하라.
  parsing.selector가 없거나, params가 비어있으면 다시 수정해서 save_recipe()하라.

■ 가격: 검색 결과에 단가 컬럼이 없으면 → 약품 클릭 → 상세 패널에서 가격 API 캡처 → product_detail 섹션에 기록.

■ 장바구니 (필수 규칙!):
  1. 담기→조회→삭제→비우기를 연속 실행하며 매 단계 get_network_log()로 API 캡처.
  2. 삭제/비우기 전에 반드시 recipe_view_cart()로 장바구니에 상품이 있는지 확인하라.
     비어있는 장바구니에 삭제/비우기를 실행하면 무한 팝업이 발생한다. 절대 하지 마라.
  3. confirm/alert 팝업이 2회 이상 반복되면 즉시 중단하라.
  4. cart_delete API 찾기: iframe이 있으면 snapshot_iframe()으로 삭제 버튼 확인.
     삭제 버튼의 코드를 execute_js로 추적하라 (iframe 안의 jQuery 이벤트: iframe.contentWindow.jQuery._data).
     href에 URL이 직접 있으면 그것을 cart_clear/cart_delete로 사용.

■ 매출원장 (필수 규칙!):
  1. 상세/요약 구분. 요약이면 상세 API도 찾아라 (행 클릭 또는 품목수불 메뉴).
  2. 날짜를 반드시 먼저 설정하라. 기본값이 아닌 최근 3일로 설정 후 조회.
     결과 0건이면 → 7일로 변경 → 재조회. 그래도 0건이면 → 14일로 변경 → 재조회.
     날짜 설정 없이 조회하면 데이터가 너무 많거나 없을 수 있다.
  3. get_page_html()로 전체 HTML을 가져오지 마라 — 너무 크다. execute_js로 테이블 헤더와 첫 3행만 확인하라.
...
■ E2E 검증 시 recipe_sales_ledger는 period="3d"(3일)로 호출하라. 결과가 0건이면 "1w"로 재시도.
...
echo "$PROMPT" | claude -p \
  ...
  --disallowedTools "Read,Edit,Write,Grep,Glob,Bash,WebSearch,WebFetch,Agent,NotebookEdit,TodoWrite" \
  2>&1 | tee "$LOG_FILE"
