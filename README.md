...
```bash
git clone https://github.com/your-username/wholesale-mcp.git
cd wholesale-mcp
python3 -m venv venv
...
```json
{
  "mcpServers": {
    "wholesale-tools": {
      "command": "/절대/경로/wholesale-mcp/venv/bin/python",
      "args": ["/절대/경로/wholesale-mcp/server.py"],
      "cwd": "/절대/경로/wholesale-mcp"
    }
  }
}
```

Claude Code 재시작 후 `list_sites()`로 연결 확인.
