...
다운로드 → 가상환경 → 의존성 → Playwright → credentials.json 생성까지 자동으로 진행됩니다.
완료 후 `.mcp.json` 설정 내용이 출력됩니다.

<details>
<summary>수동 설치 (직접 하나씩 실행하고 싶은 경우)</summary>
...
AI 도구가 이 서버를 사용하려면 `.mcp.json` 설정이 필요합니다.

**프로젝트를 다운받은 경로를 확인하세요:**
```bash
# 예: /Users/wjcho/Desktop/wholesale-mcp 에 설치했다면
cd /Users/wjcho/Desktop/wholesale-mcp
pwd  # 이 경로를 아래에서 사용
```

**`.mcp.json` 파일을 프로젝트 루트에 생성:**

Windows 예시 (`C:\Users\홍길동\wholesale-mcp`에 설치한 경우):
```json
{
  "mcpServers": {
    "wholesale-tools": {
...
macOS/Linux 예시 (`/Users/홍길동/wholesale-mcp`에 설치한 경우):
```json
{
  "mcpServers": {
    "wholesale-tools": {
...
> | 항목 | 설명 | 바꿔야 할 부분 |
> |------|------|--------------|
> | `command` | 가상환경의 python 실행파일 | `홍길동` → 본인 사용자명 |
> | `args` | server.py 파일 위치 | 동일 |
> | `cwd` | 프로젝트 폴더 (recipes/ 가 있는 곳) | 동일 |

**연결 확인:**

AI 도구를 재시작한 후 `list_sites()`를 호출해서 사이트 목록이 나오면 성공입니다.
