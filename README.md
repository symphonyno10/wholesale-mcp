...
새 도매사이트 레시피를 만들었다면 커뮤니티에 공유할 수 있습니다.
...
- **제출**: 누구나 가능 (인증 불필요)
- **검토**: 관리자가 확인 (악성 URL, 필수 필드 등)
- **배포**: 검토 통과 → pip 패키지에 포함 → `pip install --upgrade wholesale-mcp`

레시피에는 개인정보(ID/PW, 거래처코드)가 포함되지 않으므로 안전하게 공유할 수 있습니다.
