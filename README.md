...
```
사용자: "복산나이스팜 레시피 다시 만들어줘"

AI → open_site("https://wos.nicepharm.com")
   → analyze_page_for_recipe() → 사이트 재분석
   → save_recipe("wos_nicepharm_com", new_json, overwrite=True)
   → E2E 검증 (로그인/검색/장바구니/매출원장)
   → "레시피 재생성 완료. 4/4 기능 정상 작동합니다."
```

## 레시피 공유에 참여해주세요!

이 프로젝트는 **더 많은 도매사이트 레시피**가 모일수록 모든 사용자에게 도움이 됩니다.
거래하시는 도매사이트의 레시피를 만드셨다면 꼭 공유 부탁드립니다!

```
사용자: "이 레시피 공유해줘"
AI → share_recipe("new_site_com")
   → "레시피가 검토 대기열에 제출되었습니다."
```

레시피 생성이 완료되면 AI가 자동으로 공유 여부를 물어봅니다. "응"이라고만 하면 됩니다.

- **레시피에는 개인정보(ID/PW, 거래처코드)가 포함되지 않습니다** — 안심하고 공유하세요
- 제출된 레시피는 관리자 검토 후 다음 업데이트에 포함됩니다
- 공유된 레시피가 많아질수록 `pip install --upgrade wholesale-mcp`로 더 많은 도매사이트를 바로 사용할 수 있습니다
