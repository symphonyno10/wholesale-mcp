...
1. analyze_page_for_recipe("search") → 3종 동시 수집
   - forms, all_links(hidden 포함), button_actions, framework, iframes
...
2. snapshot_page()로 매출원장 UI의 모든 폼 요소 확인:
   - 날짜 입력 필드, "조회" 버튼
   - 셀렉트박스, 라디오버튼 등 조회 옵션 확인 (execute_js로 option/value 목록 읽기)
3. 기본값으로 조회 → screenshot으로 결과 확인:
   - 결과가 일자별 요약("타이레놀 외 9종")이면 → 상세 옵션으로 전환
   - 옵션 변경 → 재조회 → screenshot → 개별 약품 라인이 나오는지 확인
   - 요약/상세를 제어하는 파라미터와 각각의 값을 get_network_log로 캡처
   - 레시피에 detail_values로 기록: {{"detail": "상세값", "summary": "요약값"}}
   - 해당 파라미터에 {{DETAIL_MODE}} 변수 사용
4. "조회" 버튼의 코드 추적 → form action 또는 AJAX URL 확인
5. get_network_log()로 실제 API 캡처
6. 결과 파싱 — 컬럼 매핑을 정확히 결정:
   A) HTML 응답:
      - execute_js로 테이블 헤더(th) 목록 읽기 → 각 컬럼의 의미 파악
      - 첫 데이터 행(tr)의 각 td 텍스트와 class를 순서대로 읽기
      - 날짜, 약품명, 제조사, 수량, 단가, 매출금액, 잔액을 정확히 구분
      - 주의: "약품명"과 "제조사"가 별도 컬럼이면 product_name에 약품명 컬럼을 매핑 (제조사 아님)
      - screenshot으로 화면의 실제 값과 파싱 결과가 일치하는지 대조
   B) JSON 응답:
      - get_network_log body_preview에서 실제 키 확인
      - 각 키의 값을 보고 의미 파악 (날짜형, 약품명, 숫자 등)
      - json_mapping.fields에 정확한 키 매핑
   C) 검증:
      - 데이터 행 1개를 읽어서 product_name이 실제 약품명인지 확인
      - 제조사명이 약품명으로 잡히거나, 수량/단가가 0이면 셀렉터 수정
      - 합계/월계/누계 행이 데이터에 섞이면 해당 행을 제외하는 셀렉터로 수정
   D) 2단계 조회 확인 (중요):
      - 결과가 개별 약품인지 요약인지 반드시 확인:
        * "외 N종", "외N건" 같은 텍스트가 있으면 → 요약 목록 (개별 약품 아님)
        * 수량/단가 필드가 없거나 전부 0이면 → 요약일 가능성 높음
        * 한 행에 약품이 1개만 있고 수량/단가가 구체적이면 → 개별 약품 (정상)
      - 요약이면:
        1) 옵션(라디오/셀렉트) 전환으로 상세 모드가 있는지 먼저 확인
        2) 없으면 → 요약 행 하나를 click_element로 클릭
        3) screenshot으로 하위 상세(개별 약품 목록)가 나오는지 확인
        4) get_network_log로 상세 API 캡처
        5) 반드시 레시피에 sales_ledger_detail 섹션으로 기록 — 발견만 하고 안 넣으면 안 됨
           상세 API의 URL, method, params, response_type, json_mapping을 모두 포함
