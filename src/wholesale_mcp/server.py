...
_skipped_updates = []  # user_generated 때문에 스킵된 공식 업데이트
               ...
               str(_bundled_recipes) != str(_user_recipes):
                ...
                for _src in _bundled_recipes.glob("*.json"):
                    ...
                    if not _dst.exists():
                        ...
                        # 유저 제작(user_generated) 레시피는 건드리지 않음
                        # 번들 recipe_version이 더 높으면 업데이트
                        try:
                            _user_data = json.loads(_dst.read_text(encoding="utf-8"))
                            _bv = json.loads(_src.read_text(encoding="utf-8")).get("recipe_version", 0)
                            _uv = _user_data.get("recipe_version", 0)
                            if _user_data.get("user_generated"):
                                if _bv > _uv:
                                    _skipped_updates.append({
                                        "site_id": _user_data.get("site_id", _src.stem),
                                        "current": _uv, "available": _bv,
                                    })
                                continue
                            if _bv > _uv:
                                shutil.copy2(_src, _dst)
...
def _get_recipe(site_id: str) -> dict | None:
    """레시피 조회 (_auto/_test suffix 자동 fallback)."""
    recipes = _load_recipes()
    r = recipes.get(site_id)
    if r:
        return r
    for suffix in ('_auto', '_test'):
        r = recipes.get(site_id + suffix)
        if r:
            return r
    base_id = site_id.replace('_auto', '').replace('_test', '').rstrip('_')
    if base_id != site_id:
        r = recipes.get(base_id)
        if r:
            return r
    return None
...
def register_site(url: str, username: str, password: str, site_name: str = "") -> str:
    ...
    # credentials.json에 추가 (파일 직접 읽어서 병합 — 캐시 무효화 버그 방지)
    global _credentials, _credentials_mtime
    cred_path = DATA_DIR / "credentials.json"
    creds = {}
    if cred_path.exists():
...
async def analyze_page_for_recipe(page_type: str = "auto") -> str:
    ...
    recent_posts = [req for req in list(_engine.network_log)[-20:] if req.get("method") == "POST"]
...
def save_recipe(site_id: str, recipe_json: str, overwrite: bool = False) -> str:
    ...
    try:
        ...
        # 유저/AI 생성 레시피 태그 — mcpb 업데이트 시 덮어쓰기 방지
...
def _validate_path(file_path: str, must_exist: bool = True) -> Path:
    ...
    # 정규화 (.. 제거) — .resolve() 대신 normpath 사용 (hang 방지)
    target = Path(os.path.normpath(str(target)))

    # 샌드박스 경계 확인
    try:
        target.relative_to(Path(os.path.normpath(str(data_root))))
    except ValueError:
        raise ValueError(
...
async def capture_form_submission(form_selector: str = "form") -> str:
    ...
    new_requests = list(_engine.network_log)[before_count:]
...
7. **쿠키에 거래처 정보 확인**: 쿠키 값에 venCd, venNm 등이 key=value&key=value 형식으로 들어있으면
   cookie_parse를 레시피에 추가 (정확한 포맷: cookie_name + format: "querystring" + fields)
   ```json
   "cookie_parse": {{
     "cookie_name": "쿠키이름",
     "format": "querystring",
     "fields": {{"VEN_CD": "venCd", "VEN_NM": "venNm"}}
   }}
   ```
8. 로그인 후 screenshot() → 팝업이 UI를 가리면 "닫기"/"확인" 버튼 클릭 → 반복
...
쿠키에 거래처 코드가 들어있는 사이트 (WOS 플랫폼 등):
```json
"cookie_parse": {
  "cookie_name": "wos",
  "format": "querystring",
...
```
- cookie_name: 파싱할 쿠키 이름
- format: "querystring" (key=value&key=value 형식)
- fields: {레시피 변수명: 쿠키 키명} — 주의: 레시피 변수가 key, 쿠키 키가 value
...
# ── 일상 사용 프롬프트 (Claude Desktop에서 / 로 호출) ──

@mcp.prompt()
def 도매현황() -> str:
    """등록된 도매 사이트 현황 확인. 로그인 상태, 사용 가능 기능, 레시피 업데이트 확인."""
    return """list_sites()를 호출하여 아래 정보를 정리해주세요:
1. 각 사이트의 로그인 상태 (logged_in)
2. 사용 가능한 기능 (available_features)
3. 레시피 업데이트가 있으면 알려주세요
결과를 표 형식으로 보여주세요."""


@mcp.prompt()
def 검색(keyword: str) -> str:
    """전체 도매 사이트에서 약품 검색."""
    return f"""로그인된 모든 사이트에서 "{keyword}"를 검색해주세요.
1. list_sites()로 logged_in=true인 사이트 확인
2. 각 사이트에서 recipe_search(site_id, "{keyword}") 실행
3. 결과를 사이트별로 비교하여 표 형식으로 보여주세요 (약품명, 단가, 재고, 사이트)"""


@mcp.prompt()
def 매출분석(period: str = "3m") -> str:
    """매출원장 동기화 + 분석."""
    return f"""매출 데이터를 동기화하고 분석해주세요.
1. sync_ledger("all", "{period}")로 전체 사이트 데이터 동기화
2. ledger_summary("all", "{period}", 20)로 TOP 20 약품 확인
3. 결과를 표 형식으로 보여주세요"""


@mcp.prompt()
def 가격비교(keyword: str) -> str:
    """도매별 같은 약품 가격 비교."""
    return f""""{keyword}"의 도매별 가격을 비교해주세요.
1. ledger_compare("{keyword}")로 도매별 단가/주문량 비교
2. recipe_search로 현재 각 사이트 판매가 확인
3. 결과를 비교표로 보여주세요 (사이트, 단가, 최근 주문일, 주문량)"""
...
def share_recipe(site_id: str) -> str:
    ...
    recipe = _get_recipe(site_id)
    if not recipe:
        raise ValueError(f"레시피 없음: {site_id}. list_sites()로 확인하세요.")
...
def _auto_login_on_startup():
    """서버 시작 시 credentials 있는 모든 사이트 자동 로그인."""
    from .site_executor import SiteExecutor
    creds = _load_credentials()
    logged = []
    for site_id, cred in creds.items():
        ...
        try:
            ...
            if old:
                old.close()
            executor = SiteExecutor(recipe)
            ok = executor.login(cred["username"], cred["password"],
                                site_params=cred.get("site_params", {}))
            if ok:
                _executors[site_id] = executor
                logged.append(site_id)
                logger.info(f"자동 로그인 성공: {site_id}")
        except Exception as e:
            logger.warning(f"자동 로그인 실패: {site_id} ({e})")
    return logged

def main():
    # 자동 로그인
    logged = _auto_login_on_startup()

    # 동적 instructions 설정
    lines = [
        f"자동 로그인 완료: {len(logged)}개 사이트.",
        "list_sites()로 상태 확인. available_features 확인 후 해당 기능만 호출.",
        "새 사이트: register_site() → analyze_page_for_recipe() → save_recipe().",
    ]
    if _skipped_updates:
        sites = ", ".join(s["site_id"] for s in _skipped_updates)
        lines.append(f"공식 레시피 업데이트 있음: {sites}. get_recipe()로 확인 후 save_recipe(overwrite=True)로 적용.")
    mcp._instructions = " ".join(lines)
