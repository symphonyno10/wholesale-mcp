...
6. 결과 파싱 — 컬럼 매핑을 정확히 결정:
   ...
   D) 2단계 조회 확인 (중요):
      - screenshot으로 결과가 개별 약품 라인인지, 날짜별 요약 묶음인지 확인
      - 옵션 전환으로 상세 모드를 못 찾은 경우:
        요약 행 하나를 click_element로 클릭 → screenshot으로 하위 상세가 나오는지 확인
      - 하위 상세가 나오면 → get_network_log로 상세 API 캡처
      - 레시피에 sales_ledger_detail 섹션으로 기록 (상위 목록과 별도 API)
