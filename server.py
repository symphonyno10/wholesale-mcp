...
1. open_site("{site_url}")
2. analyze_page_for_recipe("login") → 폼 구조, 필드명 확인
3. fill_input으로 ID/PW 입력
4. capture_form_submission() → 실제 POST URL + payload 캡처
5. get_cookies() → 새로 생긴 쿠키 = success_indicator

## STEP 2: 메인 페이지 3종 분석

analyze_page_for_recipe("search") 한 번 호출로 모든 정보 파악:
- forms: 검색 폼 구조, 필드명
- all_links: 매출원장 등 hidden 메뉴 위치
- js_handlers.ajax_urls: 장바구니/검색 API URL
- js_handlers.cart_functions: AddCart, ProcessCart 등 함수명
- html_forms_raw: form 태그 원문

## STEP 3: 검색 실행 + 결과 파싱

1. fill_input으로 "타이레놀" 입력 → submit_form()
2. get_network_log() → 검색 API URL + 파라미터
3. execute_js로 결과 테이블 구조 확인 (행 셀렉터, td 클래스, hidden div)

## STEP 3.5: 페이지네이션 판별

A) JSON API 응답에 totalPage 필드 있으면 → pagination.type: "param"
B) HTML에 페이지 링크(div.paging 등) 있으면 → pagination.type: "html_links"
C) 둘 다 없으면 → pagination 섹션 없음

## STEP 4: 장바구니 방식 결정

A) js_handlers.ajax_urls에 /DataCart/ 또는 /cart/add 있으면 → API 방식
