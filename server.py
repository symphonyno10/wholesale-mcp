...
4. screenshot()으로 검색 결과 직접 확인:
   - 가격 컬럼 유무 확인 → 없으면 상품 클릭 → screenshot() → 상세 패널 가격 확인
   - get_network_log()에서 상세 API 캡처 (예: /Home/PartialProductInfo/상품코드)
   - 반드시 레시피 search 섹션에 product_detail을 추가:
     "product_detail": {{"method":"POST","url":"상세API/{{PRODUCT_CODE}}","unit_price_selector":"셀렉터"}}
5. 결과 파싱 구조:
