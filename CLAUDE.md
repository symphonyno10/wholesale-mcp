...
- JWT 사이트는 로그인 응답의 `userData`에서 필요한 값을 자동 추출 → `site_params` 불필요할 수 있음
- form POST 사이트의 매출원장은 거래처 코드가 필요할 수 있음 → `site_params`에 저장
...
3. hidden 메뉴 적극 탐색:
   ...
   - 햄버거 메뉴, 드롭다운, 사이드바 토글 버튼 클릭 → 하위 메뉴 노출
     예: "조회" 메뉴 클릭 → 매출원장, 세금계산서 등 하위 메뉴 노출
...
검증된 레시피 예시는 `recipes/` 디렉토리의 기존 레시피 파일 참조.
...
|-----------|-----|
| `forms` | 있음 (검색폼) |
| `ajax_urls` | **Cart/Search/Data 등 URL 있음** |
| `cart_functions` | 장바구니 관련 함수명 있음 |
| `all_links` | hidden 메뉴에 매출원장 등 (visible=false) |
| **인증** | 쿠키 기반 |
| **검색** | AJAX POST → HTML partial 응답 |
| **장바구니** | REST API — `execute_js`로 함수.toString() 확인하여 payload 파악 |
| **상품코드** | hidden div에 있을 수 있음 |
| **가격** | 검색 목록에 없을 수 있음, 상품 클릭 후 상세 패널에서 확인 |
...
| **상품코드** | JSON 응답의 특정 필드 |
| **가격** | JSON 응답의 특정 필드 |
| **핵심** | 로그인 응답의 `userData`에서 사용자 정보 추출 → 이후 모든 API에 변수로 사용 |
...
| `cookies` | 로그인 후 사이트별 쿠키 생성 |
| `external_js_files` | `angular.*.js` 등 |
| **인증** | 쿠키 기반 (JWT 아님) |
| **검색** | GET JSON API (네트워크 캡처로 확인) |
| **장바구니** | POST (네트워크 캡처로 확인) |
| **상품코드** | JSON 응답의 특정 필드 |
| **가격** | JSON 응답의 특정 필드 |
...
4. **Playwright fill이 Angular 모델을 업데이트 안 할 수 있음**
   ...
   ```javascript
   scope.$apply(() => { scope.모델명.id필드 = 'user'; scope.모델명.pw필드 = 'pass'; });
   scope.로그인함수();
   ```
5. 검색도 DOM input 값을 직접 설정해야 할 수 있음:
   ```javascript
   document.querySelector('검색입력셀렉터').value = '타이레놀';
   scope.검색함수();
   ```
...
```
1. open_site(login_url) → fill_input → click(로그인 버튼) → get_network_log()
   → 로그인 POST 요청 캡처

2. fill_input(검색어) → click(검색 버튼) → get_network_log()
   → 검색 API 요청 캡처

3. click(담기 버튼) → get_network_log()
   → 장바구니 추가 POST 요청 캡처

4. click(매출원장 메뉴) → get_network_log()
   → 매출원장 API 요청 캡처
```
